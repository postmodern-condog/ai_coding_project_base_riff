...
- Following explicit instructions (AGENTS.md, acceptance criteria)
...
- A developer can run `/product-spec` → `/technical-spec` → `/generate-plan` → `/phase-start 1..N` and end up with verified code, atomic commits traceable to requirements, and a clean git history.
...
- Any AI agent (or human) can pick up where work left off by reading the spec chain.
